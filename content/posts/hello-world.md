---
title: Hello World!|This is my new blog!
date: 2026-08-08T17:42:00+08:00
draft: false
tags:
  - 博客
  - 随笔
description: 你好世界
background: /images/pexels-tigran-manukyan-33097708.jpg
entry_title_font: /images/乐米简篆体.ttf
entry_title_font_file: /images/Boxing-Regular.otf
entry_title_font_name: ''
entry_title_color: '#010057'
entry_title_opacity: 80
entry_title_size: null
entry_title_effect: reveal
entry_scramble: false
entry_align: center-center
cover:
  image: /images/pexels-ömerhan-datlar-5382283.jpg
---

## 欢迎

这是我的第一篇博客文章！博客使用 [Hugo](https://gohugo.io) 搭建，采用自制的 brutalism 主题 —— 矿物单色 · 单点苔绿 · 变换驱动动效。

## 关于这个博客

这里将记录我的学习笔记、技术分享和生活感悟。敬请期待更多内容！

### 示例

```bash
hugo new content posts/我的文章.md
```

> 欢迎评论、订阅，或通过 GitHub 与我交流。
