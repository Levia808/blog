...
> 最后更新：2026-09-05 · opencode（Threads 媒体生命周期补丁：hover Promise 竞态、切页/隐藏停止预览、查看器关闭释放视频、多指触摸保护）
...
| 2026-09-05 | 本地未提交 | `moments.js` 增加 hover token，避免移出后旧 `play()` Promise 恢复播放态；轮播切页停止并复位预览；查看器关闭暂停并卸载旧视频；页面隐藏停止预览；多指触摸不触发翻页。验证范围：`node --check` 与 `hugo --minify`；本轮未完成真实 Chrome CDP 回归，未覆盖受信鼠标/触摸及真实媒体解码。 |
