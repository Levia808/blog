(function () {
  ...
  function layoutThreadsMedia(card) {
    ...
    pages.forEach(function (page) {
      var imgs = page.querySelectorAll('.th-media-item img, .th-media-item video');
  ...
  function threadsImageRatio(img) {
    ...
    if (img && img.videoWidth > 0) return img.videoWidth / img.videoHeight;
  ...
  function setThreadsVideoPosterState(video) {
    if (!video) return;
    video.pause();
    video.loop = false;
    video.muted = true;
  ...
  function startThreadsVideoHoverPreview(video) {
  ...
  listEl.addEventListener('mouseover', function (e) {
    var item = e.target.closest && e.target.closest('.threads-card .th-media-item.is-video');
    if (!item || (e.relatedTarget && item.contains(e.relatedTarget))) return;
    startThreadsVideoHoverPreview(item.querySelector('video'));
  });

  listEl.addEventListener('mouseout', function (e) {
    var item = e.target.closest && e.target.closest('.threads-card .th-media-item.is-video');
    if (!item || (e.relatedTarget && item.contains(e.relatedTarget))) return;
    setThreadsVideoPosterState(item.querySelector('video'));
  });
